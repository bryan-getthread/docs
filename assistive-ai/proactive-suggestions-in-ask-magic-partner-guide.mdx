...
When a ticket is opened, Ask Magic automatically generates a structured draft for your technician. It pulls from the ticket itself, your knowledge base, past related tickets, and any connected documentation (Hudu or IT Glue, if enabled).
...
Ask Magic generates a suggestion in two situations:
...
Ask Magic generates only one suggestion per ticket. If one already exists, the system does not create another.
...
Ask Magic only includes sections when there is real data to show. If a source has nothing relevant, that section is left out entirely — no placeholders, no filler.
